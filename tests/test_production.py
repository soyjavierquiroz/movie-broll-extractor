...
def fixture(tmp_path: Path):
    ...
    (run / "narrative_map.json").write_text(json.dumps({"segments": [{"segment_id": "N1"}]}))
    return source
...
def test_process_orchestrates_full_movie_and_reuses_same_source(monkeypatch, tmp_path):
    ...
    info = {"movie": source / "movie.mp4", "srt": source / "subtitles.srt", "run": tmp_path / "runs" / "film",
            ...
            "srt_sha256": "b" * 64, "metadata": {"video": {"fps": 24}, "duration_seconds": 1}}
    ...
    monkeypatch.setattr(production, "_events", lambda *_: [event])
    calls = []
    ...
    result = production.process(source, provider=object())
